/**
 * Drives /selftest.html in headless Chromium and prints its report.
...
const CANDIDATES = process.env.SELFTEST_URL
  ? [process.env.SELFTEST_URL]
  : ['http://127.0.0.1:5183/selftest.html', 'http://[::1]:5183/selftest.html']
...
await page.waitForFunction(() => window.__selftest !== undefined, null, { timeout: 120_000 })
...
const { report, failures } = await page.evaluate(() => ({
  ...
  failures: window.__selftest.failures,
}))
console.log(report)
await browser.close()
